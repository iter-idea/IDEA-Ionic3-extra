...
export class IDEAErrorReportingService {
  ...
  public sendReport(error: Error, forceSend?: boolean): Promise<any> {
    return new Promise((resolve, reject) => {
      if(!this.shouldSend() && !forceSend) {
        console.debug('ERROR REPORTING SUPPRESSED', this.getReport(error));
        resolve();
      } else this.http
        .post(IDEA_ERROR_REPORTING_API_URL, this.getReport(error))
        .subscribe(() => resolve(), err => { resolve() });
        // ^^^ never throw an error when reporting an error
    });
  }
